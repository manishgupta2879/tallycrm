                            ...
                            @forelse ($permissionsHierarchy as $catIndex => $category)
                                ...
                                <div class="mb-3">
                                    <div class="flex items-center gap-2 py-2 px-2 rounded hover:bg-gray-50">
                                        <input type="checkbox" id="cat_{{ $catIndex }}"
                                            class="category-toggle w-4 h-4 text-blue-600 border-gray-300 rounded focus:ring-1 focus:ring-blue-600 cursor-pointer">
                                        <label for="cat_{{ $catIndex }}"
                                            class="font-bold text-sm text-gray-800 cursor-pointer">{{ $category['category'] }}</label>
                                    </div>

                                    <!-- Subcategories container -->
                                    <div id="cat_container_{{ $catIndex }}" class="ml-6 hidden">
                                            ...
                                            <div class="mb-2">
                                                @if (!empty($subcategory['name']))
                                                    ...
                                                    <div class="flex items-center gap-2 py-1.5 px-2 rounded hover:bg-gray-50">
                                                        <input type="checkbox" id="sub_{{ $catIndex }}_{{ $subIndex }}"
                                                        ...
                                                        <label for="sub_{{ $catIndex }}_{{ $subIndex }}"
                                                            class="font-semibold text-xs text-gray-700 cursor-pointer">{{ $subcategory['name'] }}</label>
                                                    </div>
                                                @endif

                                                <!-- Items Level -->
                                                <div id="items_container_{{ $catIndex }}_{{ $subIndex }}"
                                                    class="{{ !empty($subcategory['name']) ? 'ml-6 space-y-2 hidden mt-1' : 'space-y-2 mt-1' }}">
                                                        ...
                                                        <div class="flex items-start gap-2 py-1 px-2 hover:bg-gray-50 rounded">
                                                            ...
                                                            <div class="flex-1 ml-1">
                                                                <label for="{{ $permId }}" class="cursor-pointer block">
                                                                    <span
                                                                        class="font-semibold text-xs text-gray-700">{{ $item['label'] }}</span>
                                                                </label>

                                                                <!-- Actions (Initially Hidden) -->
                                                                <div id="actions_{{ $item['module'] }}"
                                                                    class="module-actions hidden mt-2">
                                                                    <div class="flex flex-wrap gap-4">
                                                                        @foreach ($item['permissions'] as $permission)
                                                                            @foreach ($item['availableActions'] as $action)
                                                                                <label
                                                                                    class="flex items-center gap-1.5 cursor-pointer py-0.5 px-2 hover:bg-gray-100 rounded transition border border-transparent hover:border-gray-200">
                                                                                    <input type="checkbox"
                                                                                    ...
                                                                                    <span
                                                                                        class="text-xs text-gray-600 capitalize">{{ $action }}</span>
                                                                                </label>
                                                                            @endforeach
                                                                        @endforeach
                                                                    </div>
        ...
        document.querySelectorAll('.category-toggle').forEach(checkbox => {
            checkbox.addEventListener('change', function () {
                ...
                if (this.checked) {
                    ...
                    // For items that don't have subcategory wrappers
