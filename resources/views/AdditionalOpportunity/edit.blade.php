    ...
    <div class="p-4 max-w-full">
        <div class="bg-white rounded shadow-sm border border-gray-200">
            <div class="flex items-center justify-between py-2 px-3 border-b border-gray-200">
                <h6>Edit Additional Opportunity</h6>
            ...
            <div class="p-4">
                <!-- Form -->
                <form action="{{ route('additional-opportunities.update', $additionalOpportunity->id) }}" method="POST">
                    @csrf
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-3 mb-4">
                        ...
                        <div>
                            ...
                            <input type="text" name="company_name"
                                value="{{ old('company_name', $additionalOpportunity->company_name) }}"
                            ...
                            <select name="category_id" id="category_id"
                                ...
                                @foreach ($categories as $category)
                                    <option value="{{ $category->id }}"
                                        {{ old('category_id', $additionalOpportunity->category_id) == $category->id ? 'selected' : '' }}>
                                        {{ $category->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-3 col-span-1 md:col-span-2">
                            ...
                            <textarea name="description" rows="2"
                                class="w-full px-2 py-1 border border-gray-300 rounded text-xs focus:outline-none focus:ring-1 focus:ring-blue-600"
                                placeholder="Description">{{ old('description', $additionalOpportunity->description) }}</textarea>
                            @error('description')
                                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                            @enderror
                        </div>
